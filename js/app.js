(() => {
    ...
    (() => {
        ...
        function initSliders() {
            if (document.querySelector(".swiper")) new swiper_core_Swiper(".benefits__slider", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                freeMode: true,
        ...
        function createMarquee(direction, triggerClass) {
            const scrollingText = gsap.utils.toArray(`${triggerClass} .block img`);
        ...
        function horizontalLoop(items, config) {
            ...
            config = config || {};
            let totalWidth, curX, distanceToStart, distanceToLoop, item, i, tl = gsap.timeline({
            ...
            }), length = items.length, startX = items[0].offsetLeft, times = [], widths = [], xPercents = [], curIndex = 0, pixelsPerSecond = (config.speed || 1) * 100, snap = config.snap === false ? v => v : gsap.utils.snap(config.snap || 1);
            gsap.set(items, {
                xPercent: (i, el) => {
